...
const RESPONSES = {
  ...
  unknown: [
    "I can help with platform navigation, wallet analysis, reputation scoring, Web3 concepts, DAOs, STX, activity feeds, analytics, exports and saved searches. Try asking something more specific.",
    "Not sure I understood that. Try: 'What is a wallet?', 'How does reputation work?', 'What is a DAO?', 'How do I scan an address?', or 'How do I navigate the platform?'",
    "That's outside my scope right now. I specialize in AETHREON IQ and Web3 intelligence. Ask me about any feature or Web3 concept."
  ]
};
...
I can help you with:
  ...
  Compare Wallets  how to run side-by-side contributor analysis
  ...
  Exports  downloading branded intelligence cards
...
2.  Reputation Scoring  01000 score across 5 intelligence axes
3.  Compare Wallets  side-by-side contributor comparison
...
8.  Export  branded intelligence card downloads
...
`
 ...
 Sidebar (left panel on desktop, hamburger menu on mobile):
  ...
  Compare Wallets  side-by-side analysis
...
5.  Economic (max 200pts)
 ...
 8001000  Elite Contributor
 600799   Established Contributor
 400599   Active Contributor
 200399   Emerging Contributor
 0199     Early Contributor
...
`
...

  {
    ...
    reply: `
 Compare Wallets lets you analyze two contributors head-to-head in real time.
...
`
 ...
 Data sources available:
  ...
  Quests  mission-based ecosystem tasks
...
`
 ...
 Metrics available:
  ...
  Total SIPs (Stacks Improvement Proposals) & open votes
  Bounties posted
  Active grants
...
`
...

  {
    ...
    reply: `
 Export generates premium branded intelligence cards as downloadable PNG images.
...
Each utility creates its own unique visual export layout:
  ...
  Compare Wallets  dual-profile comparison card
...
`
...

  {
    ...
    reply: `
 Settings lets you personalize your AETHREON IQ experience.
 ...
 Click the  gear icon in the sidebar navigation
...
Unlike Web2 (where companies like Google, Meta and Amazon control your data and identity), Web3 uses:
  ...
  Blockchain  a shared, tamper-proof ledger of transactions and activity
...
`
...

  {
    ...
    reply: `
 A DAO (Decentralized Autonomous Organization) is an organization governed by its community through on-chain voting  no central authority, no single owner.

Key concepts:
  Governance  members vote on proposals using tokens or reputation
  ...
  Quests  mission-based engagement activities
...
`
  ...
  // Wallet concept
  {
    keys: ['what is a wallet', 'wallet', 'crypto wallet', 'digital wallet', 'web3 wallet', 'blockchain wallet'],
    reply: `A crypto wallet is your identity on the blockchain.

Unlike a physical wallet, it does not store money directly. It stores your private keys — the cryptographic proof that you own assets and have performed on-chain activity.

Key facts:
- Every wallet has a public address (like an account number) that others can see
- On the Stacks blockchain, addresses start with SP or SM
- Your wallet address records every transaction, bounty, gig, quest, grant and governance vote you participate in
- AETHREON IQ reads this public activity and turns it into a contributor intelligence profile

Think of your wallet address as your Web3 resume — everything you have done on-chain is permanently recorded and verifiable.

To analyze any wallet on AETHREON IQ: go to Wallet Intelligence, paste the STX address and tap Execute.`
  },
  //  STX / Stacks 

  {
    ...
    reply: `
 STX is the native token of the Stacks blockchain.
...
I can help you with:
  ...
  Troubleshooting  "Why is my scan showing an error?"
...
function answer(question) {
  ...
  if (!q) {
    return "Ask me anything about AETHREON IQ or Web3. Try: 'What is a wallet?', 'How does reputation work?', or 'How do I navigate the platform?'";
  }
  ...
  // Navigation intent — runs before topic matching
  const navIntent = [
    'how do i go to', 'how can i go to', 'how to go to',
    'how do i get to', 'how can i get to', 'how to get to',
    'how do i navigate', 'how do i open', 'how do i access',
    'where is', 'where can i find', 'where do i',
    'take me to', 'i want to go to', 'i want to open',
    'show me how to get', 'how to navigate'
  ];
  if (navIntent.some(n => q.includes(n))) {
    return TOPICS.find(t => t.keys.includes('navigate')).reply.trim();
  }

  // "What is" handler — match concept from the question
  // e.g. "what is a wallet" -> wallet topic, "what is a dao" -> DAO topic
  const whatIsMatch = q.match(/what(?:'s| is)(?: a| an)? (.+)/);
  if (whatIsMatch) {
    const concept = whatIsMatch[1].trim();
    for (const topic of TOPICS) {
      if (topic.keys.some(k => concept.includes(k) || k.includes(concept))) {
        return topic.reply.trim();
      }
    }
  }

  // Score each topic by keyword weight — longer/more specific keys rank higher
  let bestTopic = null;
  let bestScore = 0;
  ...
  for (const topic of TOPICS) {
    ...
    for (const k of topic.keys) {
      if (q.includes(k)) {
        score += k.length * (k.split(' ').length > 1 ? 2 : 1); // multi-word keys score double
      }
    }
    if (score > bestScore) {
      bestScore = score;
