...
if (exportButtons.length) {
  ...
  function analyticsBlock(data) {
    const tiles = [];
    const push = (k, v) => { if (v !== null && v !== undefined && v !== '') tiles.push(`<div class="stat-tile"><div class="stat-k">${escape(k)}</div><div class="stat-v">${escape(v)}</div></div>`); };
    const p = data.platform || {}; const u = data.users || {}; const g = data.gigs || {}; const q = data.quests || {}; const s = data.sips || {};
    push('Platform Users', p.totalUsers ?? u.totalUsers ?? u.total);
    push('Active Users', u.activeUsers ?? u.active);
    push('Total Gigs', g.totalGigs ?? g.total);
    push('Active Gigs', g.activeGigs ?? g.active);
    push('Total Quests', q.totalQuests ?? q.total);
    push('Completed Quests', q.completedQuests ?? q.completed);
    push('Total SIPs', s.totalSips ?? s.total);
    push('Open Votes', s.openVotes ?? s.activeVotes);
    push('Bounties Posted', p.totalBounties);
    push('Grants Active', p.activeGrants);
    return `<div class="result-card"><div class="stat-grid">${tiles.join('') || '<div class="empty-state">No analytics returned</div>'}</div></div>`;
  }
